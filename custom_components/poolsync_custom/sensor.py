...
SENSOR_DESCRIPTIONS: Tuple[Tuple[SensorEntityDescription, List[str], Optional[Callable[[Any], Any]]], ...] = (
    ...
    ), ["devices", "0", "status", "waterTemp"], None), # CORRECTED PATH
    (SensorEntityDescription(
    ...
    ), ["devices", "0", "status", "fwdCurrent"], None), # CORRECTED PATH
    (SensorEntityDescription(
        key="cell_rev_current", name="Cell Reverse Current", icon="mdi:current-dc",
        native_unit_of_measurement=UnitOfElectricCurrent.MILLIAMPERE, device_class=SensorDeviceClass.CURRENT,
        state_class=SensorStateClass.MEASUREMENT, entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["devices", "0", "status", "revCurrent"], None), # CORRECTED PATH
    (SensorEntityDescription(
        ...
        state_class=SensorStateClass.MEASUREMENT, entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["devices", "0", "status", "outVoltage"], None), # CORRECTED PATH
    (SensorEntityDescription(
    ...
    ), ["devices", "0", "system", "cellSerialNum"], None), # CORRECTED PATH
    (SensorEntityDescription(
        key="cell_firmware_version", name="Cell Firmware Version", icon="mdi:chip",
        entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["devices", "0", "system", "cellFwVersion"], None), # CORRECTED PATH
    (SensorEntityDescription(
        key="cell_hardware_version", name="Cell Hardware Version", icon="mdi:memory",
        entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["devices", "0", "system", "cellHwVersion"], None), # CORRECTED PATH
    ...
    (SensorEntityDescription(
        ...
        state_class=SensorStateClass.MEASUREMENT, entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
        suggested_display_precision=0,
    ), ["poolSync", "status", "boardTemp"], None), # Path is correct
    (SensorEntityDescription(
        ...
        state_class=SensorStateClass.MEASUREMENT, entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["poolSync", "status", "rssi"], None), # Path is correct
    (SensorEntityDescription(
    ...
    ), ["poolSync", "status", "dateTime"], lambda v: dt_util.parse_datetime(v) if isinstance(v, str) else None), # Path is correct
    (SensorEntityDescription(
        key="firmware_version", name="System Firmware Version", icon="mdi:chip",
        entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["poolSync", "system", "fwVersion"], None), # Path is correct
    (SensorEntityDescription(
        key="hardware_version", name="System Hardware Version", icon="mdi:memory",
        entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["poolSync", "system", "hwVersion"], None), # Path is correct
    (SensorEntityDescription(
        ...
        entity_registry_enabled_default=False, entity_category=EntityCategory.DIAGNOSTIC,
    ), ["poolSync", "stats", "upTimeSecs"], None), # Path is correct
)
...
async def async_setup_entry(
    ...
    if not coordinator.data or not isinstance(coordinator.data.get("poolSync"), dict): # poolSync check is for general data presence
        _LOGGER.warning("Coordinator %s: Initial data is missing or not structured as expected. Sensor setup may be incomplete.", coordinator.name)
        # We still attempt to add sensors; they will become unavailable if their specific data is missing.
    ...
    else: # Should not happen if definitions exist, but good to log
        _LOGGER.warning("No sensor descriptions found, no sensors added for %s.", coordinator.name)
